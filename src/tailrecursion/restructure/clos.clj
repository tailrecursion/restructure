...
(defn- ensure-generic
  [v name]
  (when-not (and v (-> (meta v) :restructure/generic))
...
(defn- specializer-class
  [spec]
  (:class spec))

(defn- specializer-kind
  [spec]
  (:kind spec))

(defn- safe-apply
  [f x]
  (try (f x) (catch Throwable _ false)))
...
(defn- sort-methods
  [methods args]
  (sort #(more-specific? %1 %2 args) methods))
...
(defn- combine-standard
  [methods args]
  (let [applicable (filter #(every? true? (map applicable? (:specializers %) args))
                           methods)
        ...
        base (fn [& as]
               (when-not primary-fn
                 (throw (ex-info "No applicable primary methods" {:phase :dispatch})))
...
(defn- combine-simple
  [methods args op]
  (let [primaries (sort-methods
                   (filter #(and (= :primary (:qualifier %))
                                 (every? true? (map applicable? (:specializers %) args)))
                           methods)
                   args)
...
(defn- combine-default
  ...
  (let [primaries (sort-methods
                   (filter #(and (= :primary (:qualifier %))
                           ...
                           methods)
                   args)]
    (when-let [method (first primaries)]
      (apply (:fn method) args))))
...
(defn ^:no-doc make-generic
  ...
  (let [generic (atom {:name name
    ...
    (with-meta
      (fn [& args] (invoke-generic generic args))
      {:restructure/generic true
       :generic generic
       :name name})))
...
(defmacro defmethod
  [name & body]
  (let [emit-specializer (fn [spec]
                           (cond
                             (nil? spec)
                               (throw (ex-info "Nil is not a valid specializer"
                                               {:specializer spec}))
                             (= spec :any) {:kind :any}
                             (= spec 't) {:kind :any}
                             (and (seq? spec) (= 'key= (first spec)))
                               (let [[_ k v] spec]
                                 `{:kind :key=, :key ~k, :value ~v})
                             (and (seq? spec) (= 'op (first spec)))
                               (let [v (second spec)]
                                 `{:kind :key=, :key :op, :value ~v})
                             (and (seq? spec) (= 'keys= (first spec)))
                               (let [ks (rest spec)]
                                 `{:kind :keys=, :keys ~(set ks)})
                             (and (seq? spec) (= 'has-keys (first spec)))
                               (let [ks (rest spec)]
                                 `{:kind :keys, :keys ~(vec ks)})
                             ...
                             (and (seq? spec) (= 'pred (first spec)))
                               (let [p (second spec)]
                                 (cond
                                   (symbol? p) `{:kind :pred, :pred (deref (var ~p))}
                                   :else `{:kind :pred, :pred ~p}))
                             (and (seq? spec) (= 'satisfies (first spec)))
                               (let [p (second spec)]
                                 (cond
                                   (symbol? p)
                                     `{:kind :satisfies, :protocol (deref (var ~p))}
        ...
        parsed (mapv (fn [a]
                       (cond
                         (symbol? a) {:sym a, :spec {:kind :any}}
                         (and (seq? a) (= 2 (count a)) (symbol? (first a)))
                           {:sym (first a), :spec (emit-specializer (second a))}
                         :else (throw (ex-info "Invalid method parameter" {:param a}))))
                     args)
